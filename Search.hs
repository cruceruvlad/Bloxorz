...
data Node s a = Root s Int [Node s a] | Node s a Int [Node s a] deriving Show 
...
nodeState (Node state action depth kids)= state
nodeState (Root state        depth kids)= state

nodeDepth :: Node s a -> Int
nodeDepth (Node state action depth kids) = depth 
...
createStateSpace state = Root state 0  (map (\x -> createStateSpace1 x 1) $successors state)
createStateSpace1 :: (ProblemState s a) => (a,s) -> Int-> Node s a
createStateSpace1 (action,state) depth = Node state action depth (map (\x -> createStateSpace1 x (depth + 1)) $successors state) 
...
limitedDfs1 :: (ProblemState s a, Ord s)
           => Node s a    -- Nodul stării inițiale
	   -> Int
	   -> S.Set s
           -> [Node s a]         -- Adâncimea maximă de explorare
           -> ([Node s a],S.Set s)  -- Lista de noduri

limitedDfs (Root state depth kids) maxdepth = let visited = S.fromList[state]
					      	  lst = [(Root state depth kids)]
					      in fst $foldl (\(x1,x2) y-> (if S.member (nodeState y) x2 == False && (nodeDepth y) <= maxdepth then limitedDfs1 y maxdepth (S.insert (nodeState y) x2) (x1++[y]) else (x1,x2))) (lst,visited) kids

limitedDfs1 (Node state action depth kids) maxdepth visited lst = foldl (\(x1,x2) y-> (if S.member (nodeState y) x2 == False && (nodeDepth y) <= maxdepth then limitedDfs1 y maxdepth (S.insert (nodeState y) x2) (x1++[y]) else (x1,x2))) (lst,visited) kids  
